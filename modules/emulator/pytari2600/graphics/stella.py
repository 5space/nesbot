...
class Stella(object):
    ...
    def __init__(self, clocks, inputs, AudioDriver, headless=False):
        self.clocks = clocks
        self.inputs = inputs
        ...
        self.display_lines = []
        self.display_cache = []
        for _ in range(self.END_DRAW_Y - self.START_DRAW_Y + 1):
            self.display_lines.append([self.default_color]*self.FRAME_WIDTH)
    ...
    def get_save_state(self):
        ...
        state['is_update_time']     = self.is_update_time
    ...
    def set_save_state(self, state):
        ...
        self.is_update_time           = state['is_update_time']
    ...
    def write(self, address, data):
        ...
        if False == self._is_blank:
            self._screen_scan(self.nextLine, self.display_lines)
...
#
    ...
    def _update_scans(self):
        if self.is_update_time:
            ...
            self.display_cache = numpy.copy(self.display_lines)
            # print("---BEFORE EXECUTION---")
            # print(self.display_cache)
            # print(self.clocks.system_clock)
