...
class PygameStella(stella.Stella):
    ...
    def driver_open_display(self):
        ...
        if has_numpy:
            ...
            # self.display_lines = numpy.zeros((self.END_DRAW_Y - self.START_DRAW_Y + 1, self.FRAME_WIDTH), dtype=numpy.int)
            self.display_lines = numpy.array(self.display_lines)
    ...
    def _draw_using_numpy_array(self):
        if not self.headless:
            pygame.surfarray.blit_array(self._background, self.display_lines.transpose())
        self.display_cache = numpy.copy(self.display_lines)
    ...
    def _draw_using_pixel_array(self):
        ...
        for y in range(min(self.END_DRAW_Y - self.START_DRAW_Y, self.FRAME_HEIGHT)):
            for x in range(self.FRAME_WIDTH):
                pxarray[x][y] = self.display_lines[y][x]
        del pxarray

    def _draw_using_set_at(self):
        for y in range(min(self.END_DRAW_Y - self.START_DRAW_Y, self.FRAME_HEIGHT)):
            for x in range(self.FRAME_WIDTH):
                self._background.set_at((x,y),  self.display_lines[y][x])
