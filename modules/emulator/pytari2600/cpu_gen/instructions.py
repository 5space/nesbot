...
class ReadWriteInstruction(Instruction):
    ...
    def clone(self):
        return self.__class__(self.clocks, self.pc_state, self.address, self.read, self.write, self.instruction_exec, self.additional_delay)

    def execute(self):
        if self.read.__class__.__name__ == 'NullReading':
            self.execute = self.execute_null_read
        elif self.read.__class__.__name__ == 'NullWriting':
            self.execute = self.execute_null_write
        else:
            self.execute = self.rw_execute
        self.execute()
    ...
    def rw_execute(self):
        ...
        self.clocks.system_clock += self._read_time + a.get_addressing_time()
...
class BreakInstruction(Instruction):
    ...
    def execute(self):
        ...
        self.clocks.system_clock += self.pc_state.CYCLES_TO_CLOCK
        self.memory.write(self.pc_state.S.get_value(), self.pc_state.P.get_value())
        self.pc_state.S -= 1
...
class ReturnFromInterrupt(Instruction):
    ...
    def execute(self):
        ...
        self.clocks.system_clock += self.pc_state.CYCLES_TO_CLOCK
        self.pc_state.S += 1
        self.pc_state.P.set_value(self.memory.read(self.pc_state.S.get_value()))
...
class BranchInstruction(Instruction):
    ...
    def clone(self):
        return self.__class__(self.clocks, self.pc_state, self.memory, self.condition_mask, self.condition, self.instruction_exec)

    def execute(self):
        self.clocks.system_clock += self.pc_state.CYCLES_TO_CLOCK
        if (self.pc_state.P.get_value() & self.condition_mask) == self.condition:
...
class SingleByteInstruction(Instruction):
    ...
    def clone(self):
        return self.__class__(self.clocks, self.pc_state, self.src, self.dst, self.instruction_exec)
...
class JumpInstruction(Instruction):
    ...
    def clone(self):
        return self.__class__(self.clocks, self.pc_state, self.address, self.instruction_exec)
...
class PHPInstruction(Instruction):
    ...
    def execute(self):
        ...
        self.memory.writeSp(self.pc_state.S.get_value(), self.pc_state.P.get_value())
        self.pc_state.S -= 1
        # T0 - Next kid
        self.clocks.system_clock += self.pc_state.CYCLES_TO_CLOCK
...
class PLPInstruction(Instruction):
    ...
    def execute(self):
        ...
        self.clocks.system_clock += self.pc_state.CYCLES_TO_CLOCK
        self.pc_state.S += 1
        self.pc_state.P.set_value(self.memory.readSp(self.pc_state.S.get_value()))
        # T0 - Next instruction
        self.clocks.system_clock += self.pc_state.CYCLES_TO_CLOCK
